...
CheckTopMenuDotted = function(){
	...
	if(menu.length)
	{
		menu.each(function(){
			...
			if(!afterHide) {
				do {
					...
					if(!menuItemOldSave.length) {
						...
						if((hideWidth > block_w) || $('.js-menu-appendto').length > 0) {
							break
						}
						else {
			...
			$('.js-dropdown').css('display', 'table-cell');
...
var openPhotoSwipe = function(curId) {
	var $breadsItem = $('.breadcrumbs');
	$breadsItem.css('display', 'none');
    $('html, body').css('overflow-y', 'hidden');
    var $scrollItem = $('.scroll-to-top');
	...
	for(var i = 0; i < elem.length; i++) {
		if(elem[i].nodeType==1){
			if(typeof elem[i].childNodes[1] === 'undefined') {
                var nodePos = 0;
			} else {
                var nodePos = 1;
			}
			id= elem[i].childNodes[nodePos].getAttribute('data-id');
			var imgWidth=elem[i].childNodes[nodePos].getAttribute('data-size-w');
			var imgHeight=elem[i].childNodes[nodePos].getAttribute('data-size-h');
            var $imgJBlock = $(elem[i].childNodes[nodePos]).find('img')
            if(imgWidth == null) {
                imgWidth = $imgJBlock[0].width;
			}
			if(imgHeight == null) {
            	imgHeight = $imgJBlock[0].height;
			}
			if(imgWidth>=1500){
			...
			items[id]={
		            src: elem[i].childNodes[nodePos].href,
		            w: imgWidth,
		            h: imgHeight,
		            title:  elem[i].childNodes[nodePos].getAttribute('title'),
		            msrc:elem[i].childNodes[nodePos].childNodes[nodePos].src,
		            el: elem[i]
		        };
		}
	}
...
$('.pswp').focusout(function () {
    var $breadsItem = $('.breadcrumbs');
    $breadsItem.css('display', 'block');
